...
import forms.BusinessNamePartnershipFormProvider
...
class BusinessNamePartnershipController @Inject() (
  ...
  formProvider: BusinessNamePartnershipFormProvider,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen notEnrolled andThen getData andThen requireData).async {
    implicit request =>
      ...
      renderer.render("businessNamePartnership.njk", json).map(Ok(_))
  }
      ...
      form
        ...
        .fold(
          formWithErrors => {
            ...
            renderer.render("businessNamePartnership.njk", json).map(BadRequest(_))
          },
          value =>
            for {

...
class PostCodeControllerSpec extends SpecBase with NunjucksSupport with JsonMatchers {
  ...
  "PostCode Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers    = UserAnswers(userAnswersId).set(PostCodePage, "XX99 9XX").success.value
      ...
      val filledForm = form.bind(Map("value" -> "XX99 9XX"))
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, postCodeRoute)
          .withFormUrlEncodedBody(("value", "XX99 9XX"))

...
class BusinessNamePartnershipControllerSpec extends SpecBase with NunjucksSupport with JsonMatchers {
  ...
  val formProvider = new BusinessNamePartnershipFormProvider()
  val form         = formProvider()

  lazy val businessNamePageRoute = routes.BusinessNamePartnershipController.onPageLoad(NormalMode).url
  ...
  "BusinessNamePage Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, businessNamePageRoute)
